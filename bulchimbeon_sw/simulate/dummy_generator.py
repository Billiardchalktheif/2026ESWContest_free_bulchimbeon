...
# 법정 기준: 감지기회로 전로저항은 50Ω 이하가 정상(소방시설 점검관리 매뉴얼).
# 시작값 20Ω 근처로 설정 — 열화가 진행돼도 법정 한계(50Ω)까지는 한참 남은 상태에서
# z-score가 먼저 잡아내는 것을 보여주는 게 이 시스템의 핵심 가치이므로,
# "50Ω을 넘기는 것"이 아니라 "서서히 상승하는 추세 자체"를 재현하는 데 집중한다.
# ---------------------------------------------------------------------------
FIRE_ALARM_BASELINE_OHM = 20.0
FIRE_ALARM_NOISE_STD = 0.3          # ADS1115(16bit) 기준 — 내장 12bit ADC보다 노이즈 훨씬 작음
# v3: 지속성(persistence) 조건 도입 후 재보정 — 이동평균 z-score는 순수 선형 드리프트에
# 대해 기울기가 아무리 커도 어떤 점근값 이상은 못 올라간다(server/anomaly_detection.py
# docstring 참고). 기존 0.07은 그 점근값에 못 미쳐서 3회 연속 초과 조건을 안정적으로
# 만족시키지 못했다(30번 중 11번만 alarm 도달). 0.2로 올리니 노이즈(0.3) 대비 신호가
# 충분히 커져 30/30 안정적으로 재현됨 — 실측 시뮬레이션으로 확인.
