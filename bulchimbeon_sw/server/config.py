...
# ⚠️ 회로 스케일 보정.
# 가변저항을 "열화 없음"(최소) 위치로 완전히 돌린 상태에서, A1~GND 구간의 순수
# 고정저항(100Ω 저항 실물)을 멀티미터로 직접 실측해 확정한 값 (2026-08-16, Fluke 17B+).
# 기존 242.0은 가변저항이 섞인 상태의 근사치였음 — 이번 값으로 교체.
LOOP_FIXED_OFFSET_OHM = 98.4
