...
GAS_LOSS_THRESHOLD_PCT = {
    "co2": 10.0,     # ⚠️ 소방시설 점검관리 매뉴얼 1권 10%(예외 있음) vs 2권 5% — 팀 확인 대기 중,
                     # 확정 전까지 보수적으로 낮은 값(5%) 채택 -> 10% 결정(소방시설 점검매뉴얼 기준)
...
DEFAULT_EMPTY_CONTAINER_WEIGHT_G = 260.0  # 방향제 자동분사기 무게(빈 기동용기 실측 무게)
...
# ⚠️ 회로 스케일 보정.
# 1차값(230, 2026-08-16): 당시 정상 상태 raw가 233~236Ω 사이였고 순간적으로 230Ω까지
# 떨어지는 게 관측돼, "열화 없음 = 최솟값"이라는 물리적 의미상 최솟값 근처로 잡았었다
# — 이제는 부정확했던 값으로 확인됨(아래 2차 재보정 참고).
# 2차 재보정(2026-08-26, 현재값): 자탐1(가변저항 다이얼 물리적 최소 위치, raw≈264.4Ω)과
# 자탐2(안정 상태 raw 262~263Ω)를 실측한 결과, 두 구역이 동일 회로 설계라 raw baseline이
# 262~264Ω으로 거의 일치함을 확인했다. 공용 오프셋을 이 baseline 평균(≈263.5→264)으로
# 재보정 — 자탐1/자탐2 라즈베리파이 DB 최근 40개 raw 평균 기준.
LOOP_FIXED_OFFSET_OHM = 264
