...
// ---- 설정값 (실측 후 재조정) ----
const char* WIFI_SSID = "salangwork";
const char* WIFI_PASS = "loving100";
const char* SERVER_IP = "192.168.0.13";
...
const uint8_t WIFI_CHANNEL = 6;  // 리프노드 WIFI_CHANNEL과 반드시 동일해야 함, 공유기도 이 채널로 고정 권장
...
void ensureWiFiConnected() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    WiFi.disconnect();
    WiFi.begin(WIFI_SSID, WIFI_PASS, WIFI_CHANNEL);             // 기존: WiFi.begin(WIFI_SSID, WIFI_PASS);
  }
}
