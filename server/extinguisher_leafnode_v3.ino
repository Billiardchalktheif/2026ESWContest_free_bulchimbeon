/*
  ...
  v3.1 변경 — ESP-NOW 실제 전달 확인 콜백 추가:
  esp_now_send()의 리턴값(ESP_OK)은 "드라이버에 잘 넘겼다"는 뜻이지 "상대방이
  실제로 받았다"는 보장이 아니다. 4개 노드 중 일부만 게이트웨이에 잡히는 문제를
  진단하기 위해 esp_now_register_send_cb()로 실제 ACK 수신 여부를 확인한다.
  이 버전에서는 함수 정의 순서 문제(콜백 함수가 그걸 등록하는 코드보다 파일
  아래쪽에 있어서 생기는 컴파일/프로토타입 이슈)를 피하기 위해 onSendResult()를
  setup()보다 위에 배치했다.
  ...
  워치독: 나머지 6개 노드와 동일한 패턴이다 — loop() 매 반복 맨 처음에
  esp_task_wdt_reset()을 호출해 "나 아직 살아있음"을 알린다. I2C가 멈추는 등
  loop()이 응답을 못 하면 WDT_TIMEOUT_MS 후 강제 재부팅된다.

  boot_id/seq: 항상 콜드부팅이므로(딥슬립 왕복이 없음) RTC_DATA_ATTR로 유지할
  이유가 없다 — setup()에서 매번 새로 생성한다.
  ...
  작업이 필요하다(claude/ESP-NOW_WiFi_채널_불일치_진단.md 참고).

  ⚠️ 노드마다 반드시 바꿔야 하는 값: NODE_INDEX (0~3, 아래) — 4개 보드에 각각
  다르게 설정해서 플래싱할 것. 나머지 값(gatewayAddress, WIFI_CHANNEL 등)은
  4개 보드 전부 동일해야 한다.
*/
...
#include <WiFi.h>
#include <Wire.h>
#include "esp_task_wdt.h"  // 워치독 타이머 — Fail-Soft, 나머지 6개 노드와 동일한 패턴

// ---- 노드 식별 (리프노드 4개마다 다르게 설정해서 플래싱) ----
const int NODE_INDEX = 0;   // 0~3 (ext_01~ext_04에 대응) — 보드마다 반드시 다르게!

// ---- 게이트웨이 MAC 주소 (extinguisher_gateway_v3.ino 부팅 시 시리얼 출력으로 확인한 값) ----
uint8_t gatewayAddress[6] = {0x20, 0x50, 0x0D, 0xD2, 0x25, 0x70};

// ---- ESP-NOW 채널: 게이트웨이가 연결하는 공유기 채널과 반드시 일치 ----
...
const unsigned long HEARTBEAT_INTERVAL_MS = 2000;  // 움직임 없어도 최소 1분마다 한 번 전송 60000->2000
...
// ---------------------------------------------------------------------------
// ESP-NOW 송신 결과 콜백 — setup()보다 위에 둬서 등록 시점 순서 문제를 원천 차단
// ---------------------------------------------------------------------------
void onSendResult(const wifi_tx_info_t* txInfo, esp_now_send_status_t status) {
  Serial.printf("전송 결과 콜백: %s\n", status == ESP_NOW_SEND_SUCCESS ? "성공(ACK 받음)" : "실패(ACK 없음)");
}
...
void setup() {
  ...
  Serial.printf("NODE_INDEX=%d\n", NODE_INDEX);
  Serial.printf("gatewayAddress=%02X:%02X:%02X:%02X:%02X:%02X\n",
                gatewayAddress[0], gatewayAddress[1], gatewayAddress[2],
                gatewayAddress[3], gatewayAddress[4], gatewayAddress[5]);
  Serial.printf("WIFI_CHANNEL=%d\n", WIFI_CHANNEL);
  ...
  esp_now_register_send_cb(onSendResult);  // setup() 안, esp_now_init() 성공 직후
