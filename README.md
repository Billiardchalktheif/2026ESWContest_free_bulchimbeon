...
- **소화기 모듈**: [Wokwi에서 열기](https://wokwi.com/projects/471162307837300737)
  ...
  - **시뮬레이션 한계**: 실물은 리프노드-게이트웨이 간 **ESP-NOW 무선통신**으로 연결상태를 판단하지만, Wokwi에서 무선 통신하는 보드 2개를 구현하기 어려워 **푸시버튼으로 연결끊김 상태를 대신 흉내냄**. 판정 로직(가속도 이벤트 → 연결상태 확인 → 이탈 확정)의 흐름 자체는 실물과 동일함
 
- **자탐1(차동식구역) 모듈**: [Wokwi에서 열기](https://wokwi.com/projects/471254826421447681)
  - 소스: `simulation/자탐1_wokwi/`
  -  **시뮬레이션 한계 (자세히)**:
    ...
    - **핀 번호가 실물과 다름**: 시뮬레이터의 ESP32-C3 보드가 실제 구매한 SuperMini 보드와 핀 배치가 달라서(예: GPIO5는 시뮬레이터에서 아날로그 입력 자체가 안 됨), GPIO3/GPIO4로 임시 배정했습니다. 실물 배선은 하드웨어 조감도 문서(ADS1115 A0/A1) 기준을 따릅니다.
